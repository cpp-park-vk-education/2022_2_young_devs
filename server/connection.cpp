...
StepRequest parsePostBody(std::string const &body)
{
    ...
    return { .command = command, .player_id = player_id, .value = value };
}


// =========== [ Обработка шага при игре с ботом ] ===============
...
http::response<http::string_body> handlerSetStepMulti(http::request<http::string_body> const &request)
{
    ...
    {
        roomStruct.lastReport = report;
        // уведомили второго игра об изменениях? - Нет!
...
// =========== [ Запрос обновлений с сервера - пулинг ] ===============
...
/*
    ...
    Ответ: { room_id : x, player_id : y, hash : sj6msc34v }
*/
...
/*
    Запрос: localhost/join/room/sj6msc34v
    Ответ: { room_id : x, player_id : y }
*/


// =========== [ Запрос присоединение к комнате по инвайт-ссылке ] ===============
...
// =========== [ Запрос на текущее состояние игровой комнаты для сохранения ] ===============
